...
class Model:
    ...
    def fit_model(self, train_data, save=True):
        ...
        self.history = self.model.fit(train_data['x'], train_data['y'],
                            epochs=40,
                            shuffle=True,      
                            verbose=2,
                            batch_size=250,
    ...
    def plot_training_curves(self, history=False, save=False):
        ...
        fig.tight_layout()
        if save:
          fig.savefig(f"{self.outdir}/{self.modeldir}/ntraining_curves.jpg")
    ...
    def test_model(self, test_data, save=True, verbose=False):
        ...
        results = {'idx': idx[sorted_idx], 'score': self.scores[sorted_idx], 'label': np.array(test_data['y'])[sorted_idx]}
...
def main():
    ...
    OUTDIR = 'base-model'           # Set to the name of the folder where you will save your models
    MODELDIR = '20240308--144346'    # If FINETUNE is true or Train is false and Test is true, MODELDIR must be specified as the dt_string of the model to be finetuned / tested or an error will occur
